...
export const ImportPage = ({
  ...
  const onError = (err) => {
    ...
    if (typeof err === "string" && err.includes("RequestDataTooBig")) {
      const message = t("dataImport.errors.fileTooBig");
    ...
    (files) => {
      ...
      for (const f of files) {
        if (!allSupportedExtensions.includes(getFileExtension(f.name))) {
          onError(new Error(t("dataImport.errors.unsupportedFileType", { name: f.name })));
          return;
        }
        fd.append(f.name, f);
      }
    ...
    <div className={importClass}>
      ...
      <header className="flex gap-4">
        <form
          ...
          onSubmit={onLoadURL}
        >
          <Input placeholder={t("dataImport.urlPlaceholder")} name="url" ref={urlRef} rawClassName="h-[40px]" />
          <Button variant="primary" look="outlined" type="submit" aria-label={t("dataImport.addUrl")}>
            {t("dataImport.addUrl")}
          </Button>
        </form>
        ...
        <Button
          ...
          leading={<IconUpload />}
          aria-label={t("dataImport.uploadFiles")}
        >
          {files.uploaded.length ? t("dataImport.uploadMoreFiles") : t("dataImport.uploadFiles")}
        </Button>
        ...
        <div
          className={importClass
            ...
            .toClassName()}
        >
          <span>{t("dataImport.csv.treatAs")}</span>
          <label>
            <input {...csvProps} value="tasks" checked={csvHandling === "tasks"} /> {t("dataImport.csv.tasks")}
          </label>
          <label>
            <input {...csvProps} value="ts" checked={csvHandling === "ts"} /> {t("dataImport.csv.timeSeriesOrText")}
          </label>
        </div>
        <div className={importClass.elem("status").toClassName()}>
          {files.uploaded.length ? t("dataImport.status.filesUploaded", { count: files.uploaded.length }) : ""}
        </div>
      </header>
          ...
          >
              ...
              <div className="flex gap-4 justify-center items-start w-full h-full">
                <label htmlFor="file-input" className="w-full h-full">
                  <div className={`${dropzoneClass.elem("content")} w-full`}>
                    ...
                    <dl>
                      <dt>{t("dataImport.fileTypeGroups.images")}</dt>
                      <dd>{supportedExtensions.image.join(", ")}</dd>
                      <dt>{t("dataImport.fileTypeGroups.audio")}</dt>
                      <dd>{supportedExtensions.audio.join(", ")}</dd>
                      <dt>
                        <div className="flex items-center gap-1">
                          {t("dataImport.fileTypeGroups.video")}
                          <Tooltip title={t("dataImport.videoFormatTooltip")}>
                      ...
                      <dt>{t("dataImport.fileTypeGroups.html")}</dt>
                      <dd>{supportedExtensions.html.join(", ")}</dd>
                      <dt>{t("dataImport.fileTypeGroups.text")}</dt>
                      <dd>{supportedExtensions.text.join(", ")}</dd>
                      <dt>{t("dataImport.fileTypeGroups.structuredData")}</dt>
                      <dd>{supportedExtensions.structuredData.join(", ")}</dd>
                      <dt>{t("dataImport.fileTypeGroups.pdf")}</dt>
                      <dd>{supportedExtensions.pdf.join(", ")}</dd>
                    </dl>
                    <div className="tips">
                      <b>{t("dataImport.important")}</b>
                      <ul className="mt-2 ml-4 list-disc font-normal">
                        <li>{t("dataImport.tips.cloudStorage")}</li>
                        <li>{t("dataImport.tips.pdfMultiImage")}</li>
                        <li>{t("dataImport.tips.preannotatedData")}</li>
                      </ul>
                    </div>
                  </div>
                </label>
            ...
            {showList && (
              <div className="w-full">
                <SimpleCard
                  title={t("dataImport.filesCardTitle")}
                        ...
                        <tr key={sample.url}>
                          <td>
                            <div className="flex items-center gap-2">
                              {sample.title}
                              <Badge>{t("dataImport.sampleBadge")}</Badge>
                            </div>
                          </td>
            ...
            {ff.isFF(ff.FF_JSON_PREVIEW) && (
              <div className="w-full h-full flex flex-col min-h-[400px]">
                {projectConfigured ? (
                  <SimpleCard
                    title={t("dataImport.expectedInputPreview")}
                  ...
                  >
                    {sampleConfig.data ? (
                      ...
                      <div className="w-[calc(100%-24px)] text-lg text-negative-content bg-negative-background border m-3 rounded-md border-negative-border-subtle p-4">
                        {t("dataImport.errors.sampleLoadFailed")}
                      </div>
                    ) : null}
                  </SimpleCard>
                ) : (
                    ...
                    <div className="flex flex-col items-center gap-tight">
                      ...
                      <div className="flex flex-col items-center gap-tighter">
                        ...
                        <div className="text-body-small text-neutral-content-subtler text-center">
                          ...
                          >
                            {t("dataImport.labelingConfiguration")}
                          </Button>{" "}
                          {t("dataImport.configureFirstHintSuffix")}
                        </div>
                      </div>
                    </div>
                  </SimpleCard>
